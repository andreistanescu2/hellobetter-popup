...
var _resetCss = require("the-new-css-reset/css/reset.css");
var _fontsScss = require("./fonts.scss");
var _popupSMinCss = require("../node_modules/popups/css/popupS.min.css");
var _indexScss = require("./index.scss");
const popupS = require("93e6cc0fa11e2c");
const jQuery = require("f4f3d97068633692");
const validate = require("99ff434c7b9b6582");
...
},{"the-new-css-reset/css/reset.css":"gYNPe","./fonts.scss":"jyfAa","../node_modules/popups/css/popupS.min.css":"8aC5A","./index.scss":"lJZlQ","93e6cc0fa11e2c":"fhW2e","f4f3d97068633692":"hgMhh","99ff434c7b9b6582":"2RFON"}],"gYNPe":[function() {},{}],"jyfAa":[function() {},{}],"8aC5A":[function() {},{}],"lJZlQ":[function() {},{}],"fhW2e":[function(require,module,exports) {
(function(root, factory) {
    if (typeof define === "function" && define.amd) define(factory);
    else module.exports = factory();
})(this, function() {
    "use strict";
    var isOpen = false, queue = [];
    // Match image file
    ...
    var transition = function() {
        ...
        for(t in transitions)if (transitions.hasOwnProperty(t) && el.style[t] !== undefined) {
            type = transitions[t];
            supported = true;
            break;
        }
    ...
    PopupS.prototype = {
        ...
        _open: function(options) {
            ...
            // Set default options
            for(var name in _defaults)!(name in options) && (options[name] = _defaults[name]);
            ...
            for(var fn in this)if (fn.charAt(0) === "_") this[fn] = _bind(this, this[fn]);
            //initialize if it hasn't already been done
            this._init();
            // if it is forced, close all others
            if (options.force === true) while(queue.length > 0)queue.pop();
            queue.push(options);
        ...
        _getOverlay: function() {
            if (!this.$overlayEl) this.$overlayEl = _buildDOM({
        ...
        _getLayer: function() {
            if (!this.$layerEl) this.$layerEl = _buildDOM({
                css: {
                    display: "inline-block",
                    position: "relative",
                    textAlign: "left",
         ...
         */ _create: function() {
            ...
            if (mode != "modal-ajax") this._createPopup(item);
            else this._loadContents(item);
            ...
            if (transition.supported) _bind(self.$layerEl, transition.type, transitionDone);
        },
        _createPopup: function(item) {
            ...
            htmlObj = [
                ...
                mode != "modal" && mode != "modal-ajax" && {
                    ...
                    children: mode == "prompt" || mode == "confirm" ? !this.options.flagButtonReverse ? [
                        btnCancel,
                        btnOk
                    ] : [
                        btnOk,
        ...
        _appendPopup: function() {
            ...
            if (this.$targetEl === (document.body || document.documentElement) && this.options.flagBodyScroll === false) _css(this.$targetEl, {
                overflow: "hidden"
            });
            ...
            if (!classReg(" " + this.options.baseClassName + "-open").test(this.$wrapEl.className)) this.$wrapEl.className += " " + this.options.baseClassName + "-open";
            if (!classReg(" " + this.options.baseClassName + "-open").test(this.$layerEl.childNodes[0].className)) this.$layerEl.childNodes[0].className += " " + this.options.baseClassName + "-open";
        },
        _hide: function() {
            var self = this;
            ...
            if (queue.length > 0) this._create();
                ...
                var removeWrap = function() {
                    ...
                    if (self.$targetEl === (document.body || document.documentElement) && self.options.flagBodyScroll === false) {
                        if (self.$targetEl.style.removeProperty) self.$targetEl.style.removeProperty("overflow");
                        else self.$targetEl.style.removeAttribute("overflow");
                    }
                };
                var transitionDone = function(event) {
                    event.stopPropagation();
                ...
                if (transition.supported) _on(self.$wrapEl, transition.type, transitionDone);
                else removeWrap();
         ...
         */ _loading: function(state) {
            ...
            if (state) {
                ...
                this._appendPopup();
            } else _css(this.$layerEl.childNodes[0], {
                height: null,
                width: null,
                borderRadius: null
            });
         ...
         */ _loadContents: function(item) {
            var url = item.ajax.url, str = typeof item.ajax.str != "undefined" ? item.ajax.str : "", post = typeof item.ajax.post != "undefined" ? item.ajax.post : true, self = this;
            // Match image file
            if (url.match(R_IMG)) {
         ...
         */ _ajax: function(filename, str, post, callback, beforeSend) {
            var ajax;
            if (window.XMLHttpRequest) ajax = new XMLHttpRequest(); //IE7+, Firefox, Chrome, Opera, Safari
            else if (ActiveXObject("Microsoft.XMLHTTP")) ajax = new ActiveXObject("Microsoft.XMLHTTP"); //IE6/5
            else if (ActiveXObject("Msxml2.XMLHTTP")) ajax = new ActiveXObject("Msxml2.XMLHTTP"); //other
            else {
        ...
        _okEvent: function(event) {
            ...
            if (typeof this.callbacks.onSubmit === "function") {
                if (typeof this.$input !== "undefined") this.callbacks.onSubmit.call(this, this.$input.value);
                else this.callbacks.onSubmit.call(this);
            }
        ...
        _cancelEvent: function(event) {
            ...
            if (typeof this.callbacks.onClose === "function") this.callbacks.onClose.call(this);
            this._commonEvent();
        },
     ...
     */ function _bind(ctx, fn) {
        ...
        return fn.bind ? fn.bind.apply(fn, [
            ctx
     ...
     */ function _each(obj, iterator) {
        if (obj) {
            for(var key in obj)if (obj.hasOwnProperty(key)) iterator(obj[key], key, obj);
        }
    }
    /**
     ...
     */ function _on(el, event, fn) {
        if (typeof el.addEventListener === "function") el.addEventListener(event, fn, false);
     ...
     */ function _off(el, event, fn) {
        if (typeof el.removeEventListener === "function") el.removeEventListener(event, fn, false);
        else if (el.detachEvent) el.detachEvent("on" + event, fn);
    }
    /**
     * css recursion
     *
     ...
     */ function _css(el, prop, val) {
        if (el && el.style && prop) {
            if (prop instanceof Object) for(var name in prop)_css(el, name, prop[name]);
     ...
     */ function _buildDOM(spec) {
        ...
        _each(spec, function(value, key) {
            if (key === "css") _css(el, spec.css);
            else if (key === "text") value !== null && _appendChild(el, document.createTextNode(value));
            else if (key === "html") value !== null && (el.innerHTML = value);
            else if (key in el) try {
                el[key] = value;
            } catch (e) {
                el.setAttribute(key, value);
            }
            else if (/^data-/.test(key)) el.setAttribute(key, value);
        });
        // if the children is already an HTML Element, append it to el
        if (children && children.appendChild) _appendChild(el, children);
        else if (children) {
            if (children instanceof Array) _each(children, function(value, key) {
                if (value instanceof Object) _appendChild(el, _buildDOM(value));
            });
            else if (children instanceof Object) _appendChild(el, _buildDOM(children));
        }
        return el;
    }
    /**
     ...
     */ function _autoFocus(parentNode) {
        ...
        for(; i < n; i++){
            el = items[i];
            if (el.type === "submit") !element && (element = el);
            else if (!/hidden|check|radio/.test(el.type) && el.value === "") {
                element = el;
                break;
            }
        }
        if (!element) element = _getElementsByTagName(parentNode, "button")[0];
        try {
            element.focus();
        } catch (err) {}
    }
...
},{}],"2RFON":[function(require,module,exports) {
/*!
 ...
 */ (function(factory) {
    ...
    else if (0, module.exports) module.exports = factory(require("b5a588d274625188"));
    else factory(jQuery);
})(function($) {
    $.extend($.fn, {
...
},{"b5a588d274625188":"hgMhh"}]},["412hi","8lqZg"], "8lqZg", "parcelRequire2efb")
